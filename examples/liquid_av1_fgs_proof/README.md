...
> 说明：不同 VMAF 模型/版本会导致绝对值变化；本仓库以 `artifacts/` 中的日志/JSON 为准（默认 `n_subsample=2`，默认模型 `version=vmaf_v0.6.1`）。
...
你可以通过 `-VmafModel` 切换 VMAF 模型（常见：`version=vmaf_v0.6.1`、`version=vmaf_4k_v0.6.1`）：

```powershell
pwsh .\examples\liquid_av1_fgs_proof\run.ps1 -InputPath "E:\your_video.mp4" -Frames 0 -Force -VmafModel "version=vmaf_4k_v0.6.1"
```

> 注：我们在同一 Pexels 样本上用 `version=vmaf_4k_v0.6.1`（`n_subsample=2`）复算得到：
> - Plain VMAF(mean) ≈ 85.50
> - FGS25 VMAF(mean) ≈ 83.86  
> 该结果同样表明：FGS 的 VMAF 走势与内容/模型口径强相关，不能一概而论。
